...
import { useDefectTypes, useSubProcesses } from "./useShiftModalData";
...
export function NgEntryModal({ open, onOpenChange, onSubmit, hourLabels, productId }: NgEntryModalProps) {
  ...
  const handleSubmit = async () => {
    if (!form.process_id) {
      toast.error("Workstation wajib dipilih");
      return;
    }
    if (!form.defect_type_id) {
      toast.error("Jenis NG wajib dipilih");
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Tambah NG Entry</DialogTitle>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {spLoading ? (
              <div className="text-xs text-muted-foreground py-2">Memuat data workstation...</div>
            ) : (
          ...
          <div>
            ...
            {dtLoading ? (
              <div className="text-xs text-muted-foreground py-2">Memuat kategori NG...</div>
            ) : (
                ...
                <SelectContent>
                  {defectTypes.map((dt) => (
                    <SelectItem key={dt.id} value={dt.id}>
                      {dt.category ? `[${dt.category}] ` : ""}{dt.name}{dt.product_id === null ? " (Global)" : ""}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div>
            ...
            <Input type="number" min={1} value={form.quantity} onChange={(e) => setForm({ ...form, quantity: parseInt(e.target.value, 10) || 1 })} />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {DISPOSITIONS.map((d) => (
                <Button
                  key={d.id}
                  variant={form.disposition === d.id ? "default" : "outline"}
                  size="sm"
                  onClick={() => setForm({ ...form, disposition: d.id })}
                >
                  {d.label}
                </Button>
              ))}
